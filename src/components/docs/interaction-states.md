...
pressed 스타일은 별도 설정 없이 Mossy 컴포넌트에 포함된다.

universal 컴포넌트는 `onPress`로 누름을 처리한다. Mossy 래퍼가 pressed 색상 토큰을 `modifiers`로 주입하며, 눌림 모션은 시스템 기본 press 피드백(iOS highlight, Android ripple)을 그대로 사용한다.
